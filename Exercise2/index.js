const values = {
  ...
  function getMaxItem(arr) {
      let resp = {limit: 0}
      arr.forEach((i) => {
        if(resp.limit < i.limit){
          resp = i
        }
      });
      return {
        limit: resp.limit,
        over: values[resp.over_carrier_service_id],
        under: values[resp.under_carrier_service_id]
      }
  }
  ...
  function getResult(data){
    ...
    for (const key in data) {
      ...
      result[key] = getMaxItem(element)
    }
    return result
  }
  
  console.log(getResult(json.data))